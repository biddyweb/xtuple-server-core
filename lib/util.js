...
var util = module.exports = {
  ...
  requireTask: function (phaseName, taskName) {
    ...
    try {
      ...
      util.runCmd('npm install --no-global ' + name, { retry: 2 });
    }
    return require(name);
  },
   ...
   *                               and used to modify runCmd itself:
   *                                 retry    {boolean|integer} retry 1|N times
  ...
  runCmd: function (command, options) {
    ...
    if (_.isObject(options)) {
      if (options.retry) {
        if (options.retry === true)         tries = 2;
        else if (_.isNumber(options.retry)) tries += options.retry;
      }
      cont = options.continue;
      options = _.omit(options, [ 'continue', 'retry' ]);
    }

    log.info('cmd', cmdStr);
    for (var i = 1; i <= tries; i++) {
      log.silly('cmd', 'attempt ' + i + ' of ' + tries);
      try {
        if (i > 1) log.warn('cmd', 'retrying');
        result = _.isEmpty(options) ? exec(cmdStr) : exec(cmdStr, options);
        break;
      } catch (e) {
        if (i >= tries) {
          try {
            var logfunc = cont ? log.warn : log.error;
            logfunc('cmd failed', cmdStr);
            logfunc('cmd status', e.status);
            logfunc('cmd stdout', e.stdout ? e.stdout.toString() : '[undefined]');
            logfunc('cmd stderr', e.stderr ? e.stderr.toString() : '[undefined]');
          } catch (f) {}    // don't let error handling bugs mask the real err
          if (cont === true) log.verbose('cmd', 'ignoring errors');
          else               throw e;
        }
      }
    }
